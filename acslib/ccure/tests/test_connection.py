...
from acslib.base.connection import ACSConnectionException
import requests
...
def test_ccure_connection(ccure_connection, response_w_session):
    """."""
    with patch("acslib.base.connection.ACSConnection._make_request", return_value=response_w_session):
        session_id = ccure_connection.login()
    assert session_id == ccure_connection.session_id


def test_ccure_login_error_no_config(response_w_session):
    conn = CcureConnection(config={})
    with patch("acslib.base.connection.ACSConnection._make_request", return_value=response_w_session):
        with pytest.raises(ACSConnectionException):
            conn.login()


@pytest.mark.parametrize("side_effect", [requests.URLRequired, requests.HTTPError, requests.TooManyRedirects, requests.ConnectTimeout, requests.ReadTimeout, requests.Timeout, requests.ConnectionError, requests.RequestException])
